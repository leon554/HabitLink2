...
export default function DashBoardStats() {
    ...
    const completions = Util.fetchAllMapItems(HC.habitsCompletions).reduce((s, a) => [...s, ...a], [])
    ...
    return (
        <div className="m-7 my-5 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-2 gap-x-17 gap-3">
                ...
                <InfoBox value={Math.round(Util.avgNumArr(consistencies))} text="Consistency Avg" toolTipText=""/>
                ...
                <InfoBox value={completions.length} text="Total Entries" toolTipText=""/>
                <InfoBox value={Math.min(...habitStats.map(s => s.streak))} text="Worst Streak" toolTipText=""/>
                <InfoBox value={completions.length} text="Total Completions" toolTipText=""/>
                ...
                <InfoBox value={(habitStats.map(s => s.completions)).reduce((s, a) => s+ a, 0)} text="Completions" toolTipText=""/>
                ...
                <InfoBox value={(Math.round(Util.avgNumArr(habitStats.map(s => s.validComps))))} text="Avg Valid Completions" toolTipText=""/>
            </div>
        </div>
    )
}
