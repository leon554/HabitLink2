    ...
    return (
        <div className="shadow-md shadow-gray-200 dark:shadow-none w-full relative bg-panel1 rounded-2xl font outline-1 outline-border text-title justify-center p-7 pt-5 pb-7 flex flex-col items-center gap-4">
            ...
            <div className="gap-3 gap-x-14 grid-cols-2 max-sm:grid-cols-1 grid  items-stretch w-full">
                ...
                <InfoBox value={`${Math.abs(p.validComps- p.completions)}`} text="Unscheduled Completions" toolTipText="This is the total number of times you completed a habit on a day which was not scheduled"/>
                <InfoBox value={`${Math.round((p.missedSessions + p.validComps) === 0 ? 0 : (p.missedSessions / (p.missedSessions + p.validComps)) * 100)}%`} text="Miss Rate" toolTipText="This is the percentage of scheduled days that were missed."/>
